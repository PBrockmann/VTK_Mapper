...
def slider_ratioxy_changed(value):
    ...
    my_print(__main__.option_verbose, '----------------------------')
    my_print(__main__.option_verbose,
             "Changing ratio X/Y: ", __main__.option_ratioxy)
    ...
    if value != 1:
        ...
        my_print(__main__.option_verbose, "Changing projection: ",
                 __main__.option_projection)
    else:
...
def button_redraw_newlevels_pressed():
    ...
    if ListBox6a_1.count() < 3:
        my_print(__main__.option_verbose,
                 'Warning!: Not enough levels defined')
        return

    new_levels_values = []
    for i in range(ListBox6a_1.count()):
        new_levels_values.append(float(str(ListBox6a_1.item(i).text())))

    my_print(__main__.option_verbose, 'Old levels values: ',
             __main__.levels_values[:])
    my_print(__main__.option_verbose, 'Old levels nb: ', __main__.levels_nb)
    my_print(__main__.option_verbose,
             'Old levels range: ', __main__.levels_range)
    ...
    my_print(__main__.option_verbose, 'New levels values: ',
             __main__.levels_values[:])
    my_print(__main__.option_verbose, 'New levels nb: ', __main__.levels_nb)
    my_print(__main__.option_verbose,
             'New levels range: ', __main__.levels_range)
...
def button_load_variable_pressed():
    ...
    __main__.command['argvarname'] = __main__.varname
    __main__.var = __main__.read_var_fromfile(
        __main__.varfilename, __main__.varname, verbose=1)
    __main__.var_1 = __main__.compress_var(
        __main__.var, __main__.selection_both)
...
def file_browser_clicked(link):
    ...
    TextEdit7.mimeSourceFactory().setFilePath(QStringList('.'))
...
def launch_Qt_interface(window_width, window_height):
    ...
    Win.setCentralWidget(QWidget(Win, "qt_central_widget"))
    ...
    # -----------------------------
    WinLayout = QVBoxLayout(Win.centralWidget(), 20, 20, "WinLayout")

    # -----------------------------
    WinSplitter = QSplitter(Win.centralWidget(), "WinSplitter")
    WinSplitter.setOrientation(QSplitter.Vertical)
    WinSplitter.setHandleWidth(10)

    # -----------------------------
    TabWidget = QTabWidget(WinSplitter, "TabWidget")

    # -----------------------------
    Tab3 = QWidget(TabWidget, "Tab3")
    TabWidget.insertTab(Tab3, "Files")

    Tab3Layout = QGridLayout(Tab3, 1, 1, 20, 20, "Tab3Layout")

    Point3 = QPoint(0, 0)

    FileSelect3 = QFileDialog(Tab3, "FileSelect3", 0)
    ...
    # -----------------------------
    Tab5 = QWidget(TabWidget, "Tab5")
    TabWidget.insertTab(Tab5, "Variables")

    Tab5Layout = QGridLayout(Tab5, 2, 2, 20, 20, "Tab5Layout")

    List5 = QListView(Tab5, "List5")
    ...
    for i in range(len(__main__.listvarname)):
        ...
        for j in range(len(listvaratt)):
            itema = QListViewItem(List5item, None)
    ...
    PushButton5a = QPushButton(Tab5, "PushButton5a")
    PushButton5a.setGeometry(QRect(20, 20, 150, 30))
    PushButton5a.setText("Load variable")

    Spacer5b = QSpacerItem(
    ...
    # -----------------------------
    Tab1 = QWidget(TabWidget, "Tab1")
    TabWidget.insertTab(Tab1, "Lookmarks/Positions")

    PushButton1a = QPushButton(Tab1, "PushButton1a")
    PushButton1a.setGeometry(QRect(50, 30, 150, 30))
    PushButton1a.setText("Load position")

    SpinBox1a = QSpinBox(Tab1, "SpinBox1a")
    SpinBox1a.setGeometry(QRect(50+0*(120+2), 20+2*(30+2), 120, 20))
    SpinBox1a.setMaxValue(90)
    SpinBox1a.setMinValue(10)
    SpinBox1a.setLineStep(5)
    SpinBox1a.setValue(__main__.ren1xstart*100)
    QToolTip.add(SpinBox1a, "Indicate xstart of viewport")

    SpinBox1b = QSpinBox(Tab1, "SpinBox1b")
    SpinBox1b.setGeometry(QRect(50+0*(120+2), 20+3*(30+2), 120, 20))
    SpinBox1b.setMaxValue(90)
    SpinBox1b.setMinValue(10)
    SpinBox1b.setLineStep(5)
    SpinBox1b.setValue(__main__.ren1xend*100)
    QToolTip.add(SpinBox1b, "Indicate xend of viewport")

    SpinBox1c = QSpinBox(Tab1, "SpinBox1c")
    SpinBox1c.setGeometry(QRect(50+1*(120+2), 20+2*(30+2), 120, 20))
    SpinBox1c.setMaxValue(90)
    SpinBox1c.setMinValue(10)
    SpinBox1c.setLineStep(5)
    SpinBox1c.setValue(__main__.ren1ystart*100)
    QToolTip.add(SpinBox1c, "Indicate ystart of viewport")

    SpinBox1d = QSpinBox(Tab1, "SpinBox1d")
    SpinBox1d.setGeometry(QRect(50+1*(120+2), 20+3*(30+2), 120, 20))
    SpinBox1d.setMaxValue(90)
    SpinBox1d.setMinValue(10)
    SpinBox1d.setLineStep(5)
    SpinBox1d.setValue(__main__.ren1yend*100)
    QToolTip.add(SpinBox1d, "Indicate yend of viewport")

    PushButton1e = QPushButton(Tab1, "PushButton1e")
    PushButton1e.setGeometry(QRect(50+2*(120+2), 30, 150, 30))
    PushButton1e.setText("Redraw")

    SpinBox1f = QSpinBox(Tab1, "SpinBox1f")
    SpinBox1f.setGeometry(QRect(50+2*(120+2), 20+2*(30+2), 120, 20))
    SpinBox1f.setMaxValue(10)
    SpinBox1f.setMinValue(1)
    SpinBox1f.setLineStep(1)
    SpinBox1f.setValue(int(__main__.option_ratioxy))
    QToolTip.add(SpinBox1f, "Indicate ratio X/Y")

    ComboBox1g = QComboBox(Tab1, "ComboBox1g")
    ComboBox1g.setGeometry(QRect(50+2*(120+2), 20+3*(30+2), 120, 30))
    ComboBox1g.insertItem("linear")
    ComboBox1g.insertItem("orthographic")
    ComboBox1g.setCurrentText(__main__.option_projection)

    # -----------------------------
    Tab2 = QWidget(TabWidget, "Tab2")
    TabWidget.insertTab(Tab2, "Overlays")

    # oooooooooooooooooooo
    PushButton2a = QPushButton(Tab2, "PushButton2a")
    PushButton2a.setGeometry(QRect(50, 20, 120, 30))
    PushButton2a.setText("Continents color")

    TextLabel2a = QLabel(Tab2, "TextLabel2a")
    TextLabel2a.setGeometry(QRect(10, 20+1*(30+2), 37, 20))
    TextLabel2a.setText("Width")

    Slider2a = QSlider(Tab2, "Slider2a")
    Slider2a.setGeometry(QRect(50, 20+1*(30+2), 120, 22))
    ...
    Slider2a.setOrientation(QSlider.Horizontal)
    Slider2a.setTickmarks(QSlider.Below)
    Slider2a.setTickInterval(1)
    QToolTip.add(Slider2a, "Indicate line width of continents")

    # oooooooooooooooooooo
    PushButton2b = QPushButton(Tab2, "PushButton2b")
    PushButton2b.setGeometry(QRect(50+1*(120+2), 20, 120, 30))
    PushButton2b.setText("Boundaries color")

    Slider2b = QSlider(Tab2, "Slider2b")
    Slider2b.setGeometry(QRect(50+1*(120+2), 20+1*(30+2), 120, 22))
    ...
    Slider2b.setOrientation(QSlider.Horizontal)
    Slider2b.setTickmarks(QSlider.Below)
    Slider2b.setTickInterval(1)
    QToolTip.add(Slider2b, "Indicate line width of boundaries")

    # oooooooooooooooooooo
    PushButton2c = QPushButton(Tab2, "PushButton2c")
    PushButton2c.setGeometry(QRect(50+2*(120+2), 20, 120, 30))
    PushButton2c.setText("Grid color")

    Slider2c = QSlider(Tab2, "Slider2c")
    Slider2c.setGeometry(QRect(50+2*(120+2), 20+1*(30+2), 120, 22))
    ...
    Slider2c.setOrientation(QSlider.Horizontal)
    Slider2c.setTickmarks(QSlider.Below)
    Slider2c.setTickInterval(1)
    QToolTip.add(Slider2c, "Indicate line width of grid")

    TextLabel2c = QLabel(Tab2, "TextLabel2c")
    TextLabel2c.setGeometry(QRect(10+2*(120+2), 20+2*(30+2), 37, 20))
    TextLabel2c.setText("Delta")

    SpinBox2c = QSpinBox(Tab2, "SpinBox2c")
    SpinBox2c.setGeometry(QRect(50+2*(120+2), 20+2*(30+2), 120, 20))
    SpinBox2c.setMaxValue(60)
    SpinBox2c.setMinValue(1)
    SpinBox2c.setLineStep(10)
    SpinBox2c.setValue(__main__.option_grid_delta)
    QToolTip.add(SpinBox2c, "Indicate delta of grid")

    # oooooooooooooooooooo
    PushButton2d = QPushButton(Tab2, "PushButton2d")
    PushButton2d.setGeometry(QRect(50+3*(120+2), 20, 120, 30))
    PushButton2d.setText("Equator color")

    Slider2d = QSlider(Tab2, "Slider2d")
    Slider2d.setGeometry(QRect(50+3*(120+2), 20+1*(30+2), 120, 22))
    ...
    Slider2d.setOrientation(QSlider.Horizontal)
    Slider2d.setTickmarks(QSlider.Below)
    Slider2d.setTickInterval(1)
    QToolTip.add(Slider2d, "Indicate line width of equator")

    # -----------------------------
    Tab4 = QWidget(TabWidget, "Tab4")
    TabWidget.insertTab(Tab4, "Operations")

    Tab4Layout = QGridLayout(Tab4, 1, 1, 20, 20, "Tab4Layout")

    TextEdit4a = QTextEdit(Tab4, "TextEdit4a")

    Tab4Layout.addWidget(TextEdit4a, 0, 0)

    # -----------------------------
    Tab6 = QWidget(TabWidget, "Tab6")
    TabWidget.insertTab(Tab6, "Levels/Colors")

    PushButton6_1 = QPushButton(Tab6, "PushButton6_1")
    PushButton6_1.setGeometry(QRect(50, 30, 150, 30))
    PushButton6_1.setText("Load colors")

    PushButton6_2 = QPushButton(Tab6, "PushButton6_2")
    PushButton6_2.setGeometry(QRect(210, 30, 150, 30))
    PushButton6_2.setText("Redraw")

    GroupBox6a = QGroupBox(Tab6, "GroupBox6a")
    GroupBox6a.setGeometry(QRect(50, 80, 410, 160))
    GroupBox6a.setTitle("Contour values")
    ListBox6a_1 = QListBox(GroupBox6a, "ListBox6a_1")
    ListBox6a_1.setGeometry(QRect(20, 30, 290, 110))
    ListBox6a_1.setVScrollBarMode(QListBox.AlwaysOn)
    ListBox6a_1.setHScrollBarMode(QListBox.Auto)
    for value in __main__.levels_values:
        ListBox6a_1.insertItem(str(value))
    ListBox6a_1.sort()
    PushButton6a_2 = QPushButton(GroupBox6a, "PushButton6a_2")
    PushButton6a_2.setGeometry(QRect(320, 30, 80, 30))
    PushButton6a_2.setText("delele")
    PushButton6a_3 = QPushButton(GroupBox6a, "PushButton6a_3")
    PushButton6a_3.setGeometry(QRect(320, 60, 80, 30))
    PushButton6a_3.setText("delele all")

    GroupBox6b = QGroupBox(Tab6, "GroupBox6b")
    GroupBox6b.setGeometry(QRect(50, 250, 410, 80))
    GroupBox6b.setTitle("Add value")
    TextLabel6b_1 = QLabel(GroupBox6b, "TextLabel6b_1")
    TextLabel6b_1.setGeometry(QRect(20, 30, 65, 20))
    TextLabel6b_1.setText("New value")
    Slider6b_2 = QSlider(GroupBox6b, "Slider6b_2")
    Slider6b_2.setGeometry(QRect(90, 30, 110, 20))
    Slider6b_2.setOrientation(QSlider.Horizontal)
    LineEdit6b_3 = QLineEdit(GroupBox6b, "LineEdit6b_3")
    LineEdit6b_3.setGeometry(QRect(210, 30, 90, 20))
    # LineEdit6b_3.setValidator(QDoubleValidator())
    PushButton6b_4 = QPushButton(GroupBox6b, "PushButton6b_4")
    PushButton6b_4.setGeometry(QRect(320, 30, 80, 30))
    PushButton6b_4.setText("add")

    GroupBox6c = QGroupBox(Tab6, "GroupBox6c")
    GroupBox6c.setGeometry(QRect(50, 340, 410, 110))
    GroupBox6c.setTitle("Generate range of values")
    TextLabel6c_1 = QLabel(GroupBox6c, "TextLabel6c_1")
    TextLabel6c_1.setGeometry(QRect(30, 30, 110, 20))
    TextLabel6c_1.setText("Number of values")
    Slider6c_2 = QSlider(GroupBox6c, "Slider6c_2")
    Slider6c_2.setGeometry(QRect(150, 30, 110, 20))
    Slider6c_2.setOrientation(QSlider.Horizontal)
    LineEdit6c_3 = QLineEdit(GroupBox6c, "LineEdit6c_3")
    LineEdit6c_3.setGeometry(QRect(270, 30, 30, 20))
    PushButton6c_4 = QPushButton(GroupBox6c, "PushButton6c_4")
    PushButton6c_4.setGeometry(QRect(320, 30, 80, 30))
    PushButton6c_4.setText("Generate")
    TextLabel6c_5 = QLabel(GroupBox6c, "TextLabel6c_5")
    TextLabel6c_5.setGeometry(QRect(30, 70, 40, 20))
    TextLabel6c_5.setText("Range")
    LineEdit6c_6 = QLineEdit(GroupBox6c, "LineEdit6c_6")
    LineEdit6c_6.setGeometry(QRect(80, 70, 90, 20))
    Slider6c_7 = QSlider(GroupBox6c, "Slider6c_7")
    Slider6c_7.setGeometry(QRect(180, 70, 110, 20))
    Slider6c_7.setOrientation(QSlider.Horizontal)
    LineEdit6c_8 = QLineEdit(GroupBox6c, "LineEdit6c_8")
    LineEdit6c_8.setGeometry(QRect(300, 70, 90, 20))

    # -----------------------------
    Tab7 = QWidget(TabWidget, "Tab7")
    TabWidget.insertTab(Tab7, "Texts")

    Tab7Layout = QGridLayout(Tab7, 1, 1, 20, 20, "Tab7Layout")

    TextEdit7 = QTextBrowser(Tab7, "TextEdit7")
    TextEdit7.setTextFormat(QTextEdit.AutoText)
    ...
    # -----------------------------
    Tab8 = QWidget(TabWidget, "Tab8")
    TabWidget.insertTab(Tab8, "Help")

    Tab8Layout = QGridLayout(Tab8, 1, 1, 20, 20, "Tab8Layout")

    TextBrowser8 = QTextBrowser(Tab8, "TextBrowser1")
    TextBrowser8.setText(__main__.interactions)

    Tab8Layout.addWidget(TextBrowser8, 0, 0)

    # -----------------------------
    Tab9 = QWidget(TabWidget, "Tab9")
    TabWidget.insertTab(Tab9, "Credits")

    Tab9Layout = QGridLayout(Tab9, 1, 1, 20, 20, "Tab9Layout")

    TextBrowser9 = QTextBrowser(Tab9, "TextBrowser1")
    TextBrowser9.setText(__main__.credits)

    Tab9Layout.addWidget(TextBrowser9, 0, 0)

    # -----------------------------
    Win.connect(FileSelect3, SIGNAL(
        "fileSelected(const QString&)"), button_file_selected)
    #Win.connect(FileSelect3,SIGNAL("okClicked()"), button_ok)
    #Win.connect(FileSelect3,SIGNAL("cancelClicked()"), button_cancel)

    Win.connect(PushButton1a, SIGNAL("pressed()"),
                button_load_position_pressed)
    Win.connect(SpinBox1a, SIGNAL("valueChanged(int)"),
                slider_xstart_grid_changed)
    Win.connect(SpinBox1b, SIGNAL("valueChanged(int)"),
                slider_xend_grid_changed)
    Win.connect(SpinBox1c, SIGNAL("valueChanged(int)"),
                slider_ystart_grid_changed)
    Win.connect(SpinBox1d, SIGNAL("valueChanged(int)"),
                slider_yend_grid_changed)
    Win.connect(PushButton1e, SIGNAL("pressed()"), button_redraw_pressed)
    Win.connect(SpinBox1f, SIGNAL(
        "valueChanged(int)"), slider_ratioxy_changed)
    Win.connect(ComboBox1g, SIGNAL("activated(const QString&)"),
                combobox_projection_activated)
    Win.connect(PushButton2a, SIGNAL("pressed()"),
                button_color_continents_pressed)
    Win.connect(Slider2a, SIGNAL("valueChanged(int)"),
                slider_width_continents_changed)
    Win.connect(PushButton2b, SIGNAL("pressed()"),
                button_color_boundaries_pressed)
    Win.connect(Slider2b, SIGNAL("valueChanged(int)"),
                slider_width_boundaries_changed)
    Win.connect(PushButton2c, SIGNAL(
        "pressed()"), button_color_grid_pressed)
    Win.connect(Slider2c, SIGNAL("valueChanged(int)"),
                slider_width_grid_changed)
    Win.connect(SpinBox2c, SIGNAL("valueChanged(int)"),
                slider_delta_grid_changed)
    Win.connect(PushButton2d, SIGNAL("pressed()"),
                button_color_equator_pressed)
    Win.connect(Slider2d, SIGNAL("valueChanged(int)"),
                slider_width_equator_changed)
    Win.connect(List5, SIGNAL("expanded(QListViewItem*)"),
                list_variables_expanded)
    Win.connect(PushButton5a, SIGNAL("pressed()"),
                button_load_variable_pressed)
    Win.connect(PushButton6_1, SIGNAL("pressed()"),
                button_load_spectrumferretfile_pressed)
    Win.connect(PushButton6_2, SIGNAL("pressed()"),
                button_redraw_newlevels_pressed)
    Win.connect(PushButton6a_2, SIGNAL("pressed()"), button_delete_level)
    Win.connect(PushButton6a_3, SIGNAL(
        "pressed()"), button_delete_alllevels)
    Win.connect(PushButton6b_4, SIGNAL("pressed()"), button_add_level)
    Win.connect(PushButton6c_4, SIGNAL("pressed()"), button_generate_levels)
    Win.connect(TextEdit7, SIGNAL(
        "linkClicked(const QString&)"), file_browser_clicked)
