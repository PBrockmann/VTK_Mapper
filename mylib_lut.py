...
def define_lut1(file,nbcolors) :
	...
	for line in filespk.readlines() :
		...
		if len(words) == 4 :
			a.append(string.atof(words[0]))
			r.append(string.atof(words[1]))
			g.append(string.atof(words[2]))
			b.append(string.atof(words[3]))
	filespk.close()
	...
	if nbcolors < len(a) :
		...
		my_print(option_verbose,"          than number of colors defined in the file.")
	...
	for i in range(len(a)-1):
		...
		for n in range(nvalues) :
			...
			__main__.lut1.SetTableValue(int(a[i]+n-1),newr[n]/100,newg[n]/100,newb[n]/100,1)
	#my_print(option_verbose,"%d,%4.3f,%4.3f,%4.3f"%(int(a[-1]-1),r[-1]/100,g[-1]/100,b[-1]/100))
	__main__.lut1.SetTableValue(int(a[-1]-1),r[-1]/100,g[-1]/100,b[-1]/100,1)
